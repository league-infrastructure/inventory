...
# Architecture — Version 004

> Target state after sprint 033. Changes from version 003: adds delete
> protection for scheduled backups, adds student credential fields to
> the Computer model.
...
### Backup Module (EXTENDED in v004)

**Delete protection for scheduled backups:**

- `BackupService.deleteBackup()` rejects deletion of files with `daily-`
  or `weekly-` prefix, returning an error.
- Admin backup UI hides the delete button for scheduled backups.

### Data Model (EXTENDED in v004)

**Computer model — new fields:**

- `studentUsername String?` — Student account username (default: "student")
- `studentPassword String?` — Student account password (default: "student")
- Existing `defaultUsername`/`defaultPassword` become the admin credentials
  conceptually (no rename to avoid breaking changes).

**Migration:** Adds columns and populates existing records with
`studentUsername = 'student'`, `studentPassword = 'student'`.

### Computer Detail Page (EXTENDED in v004)

- Form shows two credential sections: "Admin Credentials" (existing
  defaultUsername/defaultPassword) and "Student Credentials" (new fields).

### Label Module (EXTENDED in v004)

- Compact label now prints `studentUsername`/`studentPassword` instead of
  `defaultUsername`/`defaultPassword`.

### Previous Sprint Changes

### Label Module (EXTENDED in v003)

**Changes to `LabelService`:**

- New method `generateComputerLabel89x28(computerId)` — generates a single
  89mm × 28mm PDF label for a computer.
- New method `generateComputerBatchLabels(computerIds)` — generates a
  multi-page PDF with one 89mm × 28mm label per page.
- Existing `generateComputerLabel(computerId)` (59×102mm format) remains
  unchanged for backward compatibility.

**New label layout (89mm × 28mm):**
...
│  full  │─────────────────────────────────────────────│
│ height │ MACHINE-NAME                    (large font) │
│        │ user: student  pass: league2024              │
│        │ SN: ABC123DEF456               (small text) │
└────────┴─────────────────────────────────────────────┘
```

- QR code occupies the full height of the left side (~28mm × 28mm)
- Right side has three logical sections: header, machine info, details
- Machine name uses the hostname if available, falls back to model or ID

**New routes:**

- `GET /api/labels/computer/:id/compact` — Download single 89×28mm label
- `POST /api/labels/computers/batch` — Download batch PDF for multiple
  computers (body: `{ computerIds: number[] }`)

### Computer List UI (EXTENDED in v003)

**New columns:**

- **Checkbox** — First column, for multi-select. Header has "select all"
  checkbox.
- **Last Updated** — Shows `updatedAt` as relative date. Sortable column
  using existing `SortableHeader` component and `useTableSort` hook.

**New UI elements:**

- **Floating action bar** — Appears at the bottom of the viewport when
  one or more checkboxes are selected. Shows count of selected items and
  a "Print Labels" button. Styled as a sticky bar with shadow.
- **Print action per row** — Small printer icon button in the actions
  column for single-label printing.

**State management:**

- `selectedIds: Set<number>` state in ComputerList component
- "Select all" toggles all currently visible (filtered) computers
- Selection persists across sort changes but clears on disposition filter
  change

### Computer Detail Page (EXTENDED in v003)

**New UI elements:**

- **"Print Label" button** — Added to the action buttons area (near QR
  code display). Calls `GET /api/labels/computer/:id/compact` and opens
  the PDF in a new tab.

### API Response Changes

- Computer list endpoint (`GET /api/computers`) already returns
  `updatedAt` in the response. The frontend will start rendering it.
